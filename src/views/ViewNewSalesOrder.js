...
import {SalesOrdersUrl} from "../Helpers/UrlHelper";
...
export default function ViewNewSalesOrder() {
    ...
    return <SideMenu title="New Sales Order">
        <PartialHeader title="New Sales Order" icon={<div className="p-1.5 rounded-full bg-green-50">
        ...
        <div className="w-full mt-4 border-gray-200 bg-white">
            ...
            <div className="grid gap-6 grid-cols-2 mt-4 p-4 w-3/4 mx-auto">
                <div>
                    <TextInput value={orderId} onChange={(e) => setOrderId(e.target.value)} required={true}
                               label="Sales Order#" placeholder="Sales Order#"/>
                </div>
                <div>
                ...
                <div>
                    <DatePicker value={orderDate} onChange={(e) => setOrderDate(e)} label="Order Date" required={true}
            ...
            <div className="bg-white">
                ...
                <div className="flex items-center justify-between p-4">
                    ...
                    <ActionIcon
                        ...
                        radius={100}
                        variant={"filled"}
                        onClick={() => setOpenDrawer(false)}><XCircle/></ActionIcon>
                </div>

                {/*    avatar and name*/}
                <div className="flex items-center space-x-4 p-4 bg-gray-100">
                    <div className="shadow-md rounded-full">
                        <Avatar src={customerDetails?.CustomerAvatar || AvatarImg} className="h-16 w-16 object-cover" alt={""}/>
                    </div>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
    ...
    async function handleSave() {
        ...
        const result = await postAxios('sales/orders/new', payload).catch(ajaxError).finally(() => {
            setIsWorking(false)
        });
        if (result) {
            let d = data.push(result);
            setData(d);
            history.push(SalesOrdersUrl);
        }
    }
}