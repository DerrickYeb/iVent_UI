...
export default function ViewCategoriesManagement() {
    ...
    async function fetchData() {
        const result = await getAxios(`categories/get/categories`).catch(ajaxError);
        if (result) {
    ...
    async function handleOnUpdate() {
        ...
        if (!name) return toast("Category name is required", "info");
        setIsWorking(true);
        const result = await getAxios(`categories/update/category?id=${selectedId}&name=${DOMPurify.sanitize(name)}`).catch((err) => {
            setIsWorking(false);
            ajaxError(err);
        })
        if (result) {
            ...
            const findIndex = data.findIndex((x) => x.CategoryId === selectedId);
            if (findIndex !== -1) {
                cloneArr[findIndex].CategoryName = DOMPurify.sanitize(name);
                setData(cloneArr);
                setOpen(false)
            }
        }
    ...
    return <SideMenu>
        <PartialHeader title="Stock Categories Management" icon={<InventorySvg/>}/>
        <div className="w-full border-t border-gray-200">
            <div>
                ...
                {data && data.length > 0 ? <GridHelper data={data}>
                    <ColumnsDirective>
                        <ColumnDirective field="CategoryId" headerText="CategoryId" visible={false} isPrimaryKey/>
    ...
    function actionTpl(props) {
        return <Button size="xs" variant="light" color="teal"
                       onClick={() => handleSetForUpdate(props?.CategoryId, props?.CategoryName)}
        >Edit</Button>
    }
